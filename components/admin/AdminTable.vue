          ...
          <td v-if="hasActions">
            <div class="actions">
              ...
              <UIButton
                ...
                size="small"
                variant="primary"
                @click="$emit('changeRole', row.id, row.role === 'admin' ? 'user' : 'admin')"
              />
              <UIButton
                v-if="canUpdateStatus"
                :label="row.status === 'ordered' ? 'Process' : row.status === 'processing' ? 'Ship' : 'Complete'"
                size="small"
                variant="primary"
                @click="$emit('updateStatus', row.id, nextStatus(row.status))"
              />
              <UIButton
                v-if="canFulfill"
                label="Fulfill"
