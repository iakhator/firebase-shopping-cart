import process from 'node:process';globalThis._importMeta_=globalThis._importMeta_||{url:"file:///_entry.js",env:process.env};import dayjs from 'dayjs';
...
const Handlebars = /*@__PURE__*/getDefaultExportFromCjs(lib);
...
async function renderTemplate(name, data) {
  ...
  const [htmlContent, txtContent] = await Promise.all([
    fs$1.readFile(path.resolve(`${basePath}/${name}.html`), "utf-8"),
    fs$1.readFile(path.resolve(`${basePath}/${name}.txt`), "utf-8")
  ]);
...
const require2 = createRequire(globalThis._importMeta_.url);
...
async function sendOrderConfirmation({
  ...
  const data = {
    orderId,
    customerName,
    orderDate: dayjs((/* @__PURE__ */ new Date()).toISOString()).format("DD/MM/YYYY"),
    shippingAddress,
...
async function sendEmailVerificationLink({ email, pin, firstName }) {
  const data = {
    email,
    pin,
    firstName
...
export { sendOrderConfirmation as a, sendEmailVerificationLink as s };
//# sourceMappingURL=emailService.mjs.map
